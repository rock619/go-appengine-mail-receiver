...
const copyFile = async (srcBucketName, srcFilename, destBucketName, destFilename) => {
...
exports.onFinalizeRawEml = functions
  .region('asia-northeast1')
  .storage.bucket(srcBucket)
  .object()
  .onFinalize(async (object) => {
    await copyFile(object.bucket, object.name, destBucket, object.name).catch((err) => {
      console.error(err);
    });
  });
